...
/*
Extra
Daniel Rodríguez 01052026
Ejercicio 6: Framework de Eventos — Observer + Strategy + DI
⚠️ Este ejercicio es opcional. El tema de Observer no forma parte del temario obligatorio. Si ya lo realizaste, ¡excelente! Cuenta como puntos extra. Si no, puedes omitirlo sin afectar tu evaluación.

Objetivo
Integrar los patrones Observer, Strategy e inyección de dependencias por constructor en un sistema de eventos.

Requisitos
Crear enum EventType con ORDER_PLACED, ORDER_SHIPPED, ORDER_CANCELLED.
Crear interfaz funcional EventHandler con método handle(String eventData).
Crear clase EventBus (Observer) con subscribe(EventType, EventHandler) y publish(EventType, String).
Crear interfaz NotificationStrategy con send(String message).
Implementar 3 estrategias: EmailStrategy, SmsStrategy, SlackStrategy.
Crear OrderService que reciba EventBus y NotificationStrategy por constructor (DI).
Crear OrderProcessingException (checked) para montos inválidos.
* */
import java.util.*;
...
class EventBus {
    ...
    public void publish(EventType type, String data) {
        ...
        listeners.get(type).forEach(x -> x.handle(data));
    }
}
